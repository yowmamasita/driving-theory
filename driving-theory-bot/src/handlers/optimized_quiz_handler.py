...
class OptimizedQuizHandler:
    """
    Optimized quiz handler that works with the optimized database and question loader.
    Designed for handling thousands of concurrent users.
    """
    ...
    async def restore_sessions(self):
        ...
        try:
            ...
            for session in sessions:
                ...
                question_data = json.loads(session['current_question'])
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with user_lock:
            ...
            if session and session['awaiting_answer']:
                ...
                self.active_questions[user_id] = question_data
                self.awaiting_answer.add(user_id)
                context.user_data['language'] = session['language']
            ...
            if not question:
                ...
                if session:
                    ...
                    if question_data:
                        ...
                        self.active_questions[user_id] = question_data
                        self.awaiting_answer.add(user_id)
                        context.user_data['language'] = session.get('language', 'english')
