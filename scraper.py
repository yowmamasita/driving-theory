#!/usr/bin/env python3
"""
Driving Theory Questions Scraper for clickclickdrive.de
Scrapes all questions and answers from the driving theory catalog
"""
...
class DrivingTheoryScraper:
    ...
    def scrape_themes(self) -> List[Dict[str, str]]:
        """Scrape all themes from the index page"""
        url = f"{self.base_url}/fragenkatalog/en"
    ...
    def scrape_chapters(self, theme_url: str) -> List[Dict[str, str]]:
        ...
        chapters = []
        # Look for all links that match chapter pattern
        links = soup.find_all('a', href=re.compile(r'/fragenkatalog/en/[\d\.]+-[^/]+/[\d\.]+-[^/]+$'))
        ...
        for link in links:
            ...
            if chapter_url:
                ...
                if number_span:
                    ...
                    # Remove "Chapter" prefix if it exists
    ...
    def scrape_questions(self, chapter_url: str) -> List[Dict[str, str]]:
        ...
        questions = []
        # Look for all links that match question pattern (ends with question ID like 1.1.01-001 or 1.1.06-005-M)
        links = soup.find_all('a', href=re.compile(r'/fragenkatalog/en/[^/]+/[^/]+/[\d\.]+-\d+(?:-[A-Z])?$'))
        ...
        try:
            ...
            for info in question_info:
                ...
                if text.startswith('Question:'):
                    question_number = text.replace('Question:', '').strip()
                elif 'Points' in text or 'Point' in text:
                    points = text
    ...
    def export_json(self, filename: str = "driving_theory_questions.json"):
        """Export data to JSON format"""
        with open(filename, 'w', encoding='utf-8') as f:
            json.dump(self.data, f, indent=2, ensure_ascii=False)
        print(f"Exported to {filename}")
    
    def export_csv(self, filename: str = "driving_theory_questions.csv"):
    ...
    def export_markdown(self, filename: str = "driving_theory_questions.md"):
        """Export data to Markdown format"""
        with open(filename, 'w', encoding='utf-8') as f:
            f.write("# Driving Theory Questions\n\n")
